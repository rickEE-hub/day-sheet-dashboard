...
- `worker/wrangler.toml` has `[triggers] crons = ["0 * * * *"]` — fires **hourly**, in UTC (Cloudflare Cron Triggers have no timezone support).
- The Worker's exported `scheduled(event, env, ctx)` handler (in `worker/src/index.js`) only actually does anything on hours in `REFRESH_HOURS = new Set([8, 11, 14, 17])` — checked against Sydney *local* time via `sydneyCurrentHour()`, not the UTC hour the cron fired on. Rick wants business-hours-only refreshes (no point refreshing overnight), and a fixed UTC cron expression can't encode "8am–5pm Sydney" without drifting an hour off across the AEST/AEDT (DST) boundary — checking the local hour in code instead keeps it correct year-round. When the hour matches, it calls `refreshSchedule(env)`, which runs `buildSchedule(env)` — a straight port of the "Rentman fetch recipe" below into JS, hitting `https://api.rentman.net` directly with `fetch()` — and writes the result to KV key `"schedule"`. On any error (bad token, Rentman down, etc.) the KV write is skipped entirely and the error is instead written to KV key `"schedule:lastError"`; the last good schedule is never clobbered by a failed run.
- **This is not a cost-saving measure** — Cloudflare Cron Trigger invocations are billed as ordinary Worker requests (not against the Git-integration builds/month limit), so hourly-with-a-skip vs. every-3-hours-always is a negligible difference either way. The business-hours restriction is purely "don't bother refreshing when nothing's happening overnight," at Rick's request (2026-09-08).
...
- `POST /api/schedule` (passcode-gated, same convention as the other writes) forces an immediate refresh instead of waiting for the next cron tick, **any time of day** — the `REFRESH_HOURS` gate only applies inside `scheduled()`, not this endpoint. `worker/public/index.html` (team/edit page only, not `view.html`) has a **"Refresh now" button** in the masthead button row wired to this — click it, it POSTs, shows "Refreshing…", then reports success or the specific Rentman error via the existing save-flag banner. This is the answer to "how do I manually refresh" — no console/curl needed day-to-day.
- The date-window math (`sydneyTodayKey`, `sydneyMidnightUtcIso` in `worker/src/index.js`) is timezone-aware via `Intl` — it does not hardcode Sydney's UTC+10 offset, because that would silently go wrong once AEDT (UTC+11) starts. Don't "simplify" it back to a fixed offset. `sydneyCurrentHour()` (used by the business-hours gate above) follows the same pattern.
