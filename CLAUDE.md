...
- **Pushes go through the GitHub connector's MCP tools (`mcp__github__*`), not `git push` from the sandbox** — confirmed working 2026-09-08 (`get_me` resolves to `rickEE-hub`, `push_files`/`create_or_update_file` etc. work, including `create_branch` once the Claude GitHub App's permission set was re-approved by Rick). Earlier sessions tried raw `git push` and hit a bare 401/403 with no credential available in the sandbox even after unsetting CCR's git proxy env vars; that path is not worth retrying. `git clone`/`git fetch` from the sandbox still work fine (read-only), just not push.
